...
-- the "Class" library we're using will alllow us to represent anything in
...
-- our Ball class, which isn't much diferente than a Paddle structure-wise
...
function love.load()
	...
	-- initialize our player paddles: make the global so taht they can
	-- be detected by other functions and modules
...
function love.update(dt)
	if gameState == 'play' then		
		-- detect ball colision with paddles, reversing dxxx if true and
		-- slightly increasing it, then altering the dy based on the position of collision
		if ball:collides(player1) then
			ball.dx = -ball.dx * 1.03
			ball.x = player1.x + 5
			-- 5 is the width of the paddle because we am from the left side
		
			-- keep velocity going in the sme direction, but randomize it
			if ball.dy < 0 then
		...
		if ball:collides(player2) then
			ball.dx = -ball.dx * 1.03
			ball.x = player2.x - 4
			-- 4 is the width of the ball because we came from the right side
...
function love.draw()
	...
	if gameState == 'start' then
		love.graphics.printf('Hello Start State!', 0, 5, VIRTUAL_WIDTH, 'center')
	else
		love.graphics.printf('Hello Play State!', 0, 5, VIRTUAL_WIDTH, 'center')
	end

	-- draw score on the left and right center of the screen
	-- need to switch font to draw before actually printing
	love.graphics.setFont(scoreFont)
	love.graphics.print(tostring(player1Score), VIRTUAL_WIDTH / 2 - 80, VIRTUAL_HEIGHT / 20)
	love.graphics.print(tostring(player2Score), VIRTUAL_WIDTH / 2 + 60, VIRTUAL_HEIGHT / 20)
